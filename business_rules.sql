...
values('check-test', 'sql', 'If message appears it is ok.:::: Se il messaggio e visibile e bouna risposta.');

insert into system.br_definition(br_id, active_from, active_until, body) 
values('check-test', now(), 'infinity', 'select 2={var}');
...
'SELECT to_char(now(), ''yymm'') || trim(to_char(nextval(''application.application_nr_seq''), ''0000'')) AS nr');
...
'SELECT to_char(now(), ''yymmdd'') || ''-'' || trim(to_char(nextval(''administrative.rrr_nr_seq''), ''0000'')) AS nr');
...
'SELECT to_char(now(), ''yymmdd'') || ''-'' || trim(to_char(nextval(''source.source_la_nr_seq''), ''000000000'')) AS nr');
...
'select coalesce(St_GeometryType(ST_Union(co.geom_polygon)), ''ST_Polygon'') = ''ST_Polygon''
...
values('target-and-new-union-the-same', now(), 'infinity', 
 'select coalesce(st_equals(geom_to_snap,target_geom), true)
...
values('new-cadastre-objects-dont-overlap', now(), 'infinity', 
 'select coalesce(st_area(st_union(co.geom_polygon)) = sum(st_area(co.geom_polygon)), true)
from cadastre.cadastre_object co where transaction_id = {id}
 ');
...
values('survey-points-present', now(), 'infinity', 
 'select count (*) > 2 from cadastre.survey_point where transaction_id= {id}');
...
values('target-parcels-present', now(), 'infinity', 
 'select count (*) > 0 from cadastre.cadastre_object_target where transaction_id= {id}');
...
values('documents-present', now(), 'infinity', 
 'select count (*) > 0 from cadastre.transaction_source where transaction_id= {id}');
...
values('new-cadastre-objects-present', now(), 'infinity', 
 'select count (*) > 0 from cadastre.cadastre_object where transaction_id= {id}');
...
'SELECT (total_value < mortgage_amount) AS mortgage_check from application.application_property 
	INNER JOIN administrative.ba_unit ON application.application_property.name_firstpart || ''/'' || application.application_property.name_lastpart = administrative.ba_unit.name
	INNER JOIN administrative.rrr ON administrative.rrr.ba_unit_id = administrative.ba_unit.id
WHERE application.application_property.application_id = {id}');
----------------------------------------------------------------------------------------------------
...
'Select ABS(1 - SUM(TO_NUMBER(TO_CHAR(nominator, ''9.99999''), ''9.99999'')/TO_NUMBER(TO_CHAR(denominator, ''9.99999''), ''9.99999''))) < 0.01 FROM administrative.ba_unit
	INNER JOIN administrative.rrr ON administrative.rrr.ba_unit_id = administrative.ba_unit.id
...
'SELECT COUNT(*) > 0 FROM administrative.rrr 
WHERE ba_unit_id = {id}
...
'Select count(*) > 0 FROM application.application
...
'SELECT COUNT(*) > 1 FROM administrative.rrr 
WHERE ba_unit_id = {id}
...
'SELECT (abs(sum(administrative.ba_unit_area.size) - sum(cadastre.spatial_value_area.size))/sum(administrative.ba_unit_area.size)) < 0.01 FROM administrative.ba_unit_area 
...
'SELECT count(*) != 0 FROM administrative.ba_unit 
	 INNER JOIN administrative.ba_unit_contains_spatial_unit ON administrative.ba_unit.id = administrative.ba_unit_contains_spatial_unit.ba_unit_id
	 INNER JOIN cadastre.spatial_unit ON administrative.ba_unit_contains_spatial_unit.spatial_unit_id = cadastre.spatial_unit.id
	 INNER JOIN cadastre.cadastre_object ON cadastre.spatial_unit.id = cadastre.cadastre_object.id
WHERE administrative.ba_unit.id = {id}');		   
...
'Select Count(*) = 1 FROM cadastre.cadastre_object
...
----------------------------------------------------------------------------------------------------
insert into system.br(id, technical_type_code, feedback, description) 
values('app-title-has-primary-right', 'sql', 'Title must have a primary right::::ITALIANO',
 '{id}(application.application.id) is requested');
...
values('application-br8-check-has-services', 'critical', 'lodge', 'application', 1);

insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('application-br7-check-sources-have-documents', 'warning', 'lodge', 'application', 2);

insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('application-br1-check-required-sources-are-present', 'critical', 'lodge', 'application', 3);

insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('application-br2-check-title-documents-not-old', 'medium', 'lodge', 'application', 4);

insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('application-br3-check-properties-are-not-historic', 'critical', 'lodge', 'application', 5);

insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('application-br4-check-sources-date-not-in-the-future', 'warning', 'lodge', 'application', 6);

insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('application-br5-check-there-are-front-desk-services', 'warning', 'lodge', 'application', 7);

insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('application-br6-check-new-title-service-is-needed', 'warning', 'lodge', 'application', 8);
...
insert into system.br_validation(br_id, severity_code, target_reg_moment, target_code, order_of_execution) 
values('survey-points-present', 'warning', 'pending', 'cadastre_object', 1);
...
insert into system.br_validation(br_id, severity_code, target_reg_moment, target_code, order_of_execution) 
values('target-parcels-check-isapolygon', 'critical', 'pending', 'cadastre_object', 4);

insert into system.br_validation(br_id, severity_code, target_reg_moment, target_code, order_of_execution) 
values('documents-present', 'warning', 'pending', 'cadastre_object', 5);

insert into system.br_validation(br_id, severity_code, target_reg_moment, target_code, order_of_execution) 
values('new-cadastre-objects-present', 'warning', 'pending', 'cadastre_object', 6);

insert into system.br_validation(br_id, severity_code, target_reg_moment, target_code, order_of_execution) 
values('target-and-new-union-the-same', 'warning', 'pending', 'cadastre_object', 7);

insert into system.br_validation(br_id, severity_code, target_reg_moment, target_code, order_of_execution) 
values('new-cadastre-objects-dont-overlap', 'warning', 'pending', 'cadastre_object', 8);

insert into system.br_validation(br_id, severity_code, target_reg_moment, target_code, order_of_execution) 
values('area-check-percentage-newofficialarea-calculatednewarea', 'warning', 'pending', 'cadastre_object', 10);

insert into system.br_validation(br_id, severity_code, target_reg_moment, target_code, order_of_execution) 
values('cadastre-object-check-name', 'medium', 'pending', 'cadastre_object', 11);

insert into system.br_validation(br_id, severity_code, target_reg_moment, target_code, order_of_execution) 
values('area-check-percentage-newareas-oldareas', 'warning', 'pending', 'cadastre_object', 12);

---------------------------------CADASTRE CHANGE - APPROVE -----------------------------------------
insert into system.br_validation(br_id, severity_code, target_reg_moment, target_code, order_of_execution) 
values('survey-points-present', 'critical', 'current', 'cadastre_object', 1);
...
insert into system.br_validation(br_id, severity_code, target_reg_moment, target_code, order_of_execution) 
values('target-parcels-check-isapolygon', 'critical', 'current', 'cadastre_object', 4);

insert into system.br_validation(br_id, severity_code, target_reg_moment, target_code, order_of_execution) 
values('documents-present', 'warning', 'current', 'cadastre_object', 5);

insert into system.br_validation(br_id, severity_code, target_reg_moment, target_code, order_of_execution) 
values('new-cadastre-objects-present', 'critical', 'current', 'cadastre_object', 6);

insert into system.br_validation(br_id, severity_code, target_reg_moment, target_code, order_of_execution) 
values('target-and-new-union-the-same', 'critical', 'current', 'cadastre_object', 7);

insert into system.br_validation(br_id, severity_code, target_reg_moment, target_code, order_of_execution) 
values('new-cadastre-objects-dont-overlap', 'critical', 'current', 'cadastre_object', 8);

insert into system.br_validation(br_id, severity_code, target_reg_moment, target_code, order_of_execution) 
values('area-check-percentage-newofficialarea-calculatednewarea', 'warning', 'current', 'cadastre_object', 10);

insert into system.br_validation(br_id, severity_code, target_reg_moment, target_code, order_of_execution) 
values('cadastre-object-check-name', 'medium', 'current', 'cadastre_object', 11);

insert into system.br_validation(br_id, severity_code, target_reg_moment, target_code, order_of_execution) 
values('area-check-percentage-newareas-oldareas', 'warning', 'current', 'cadastre_object', 12);

-----------END  Cadastre Change [Floss 634 (subtask of Floss 555)]-----------------------------------------------------------------------------------------
-------------Start ba_unit and other business Validation Rules - Neil 01 November 2011-------------------------
insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('baunit-check-name', 'medium', 'lodge', 'application', 9);

insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('application-check-no-previous-digital-title-service', 'warning', 'lodge', 'application', 10);

insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('mortgage-value-check', 'warning', 'lodge', 'application', 2);
...
insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('current-rrr-for-variation-or-cancellation-check', 'medium', 'lodge', 'application', 11);

insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('applicant-name-to-owner-name-check', 'warning', 'lodge', 'application', 25);

insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('applicant-identification-check', 'medium', 'approve', 'application', 13);

insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('baunit-has-multiple-mortgages', 'warning', 'lodge', 'application', 1);

insert into system.br_validation(br_id, severity_code, target_reg_moment, target_code, order_of_execution) 
values('baunit-has-several-mortgages-with-same-rank', 'critical', 'current', 'rrr', 1);
...
insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('app-shares-total-check', 'critical', 'lodge', 'application', 16);

insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('app-current-caveat-check', 'medium', 'lodge', 'application', 1);

insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('app-current-caveat-and-no-withdrawal-or-waiver', 'critical', 'lodge', 'application', 21);

insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('app-for-title-with-mortgage', 'warning', 'lodge', 'application', 18);

insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('app-has-title-several-mortgages-with-same-rank', 'critical', 'lodge', 'application', 19);

insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('app-title-has-primary-right', 'critical', 'lodge', 'application', 1);

insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('app-area-comparison', 'medium', 'lodge', 'application', 25);

insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('app-title-has-cadastre-object', 'medium', 'lodge', 'application', 3);

insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('app-title-has-compatible-cadastre-object', 'medium', 'lodge', 'application', 23);

insert into system.br_validation(br_id, severity_code, target_application_moment, target_code, order_of_execution) 
values('app-cadastre-object-name', 'medium', 'lodge', 'application', 5);
