...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('area-check-percentage-newofficialarea-calculatednewarea', 'sql', 'The difference between the new official parcel area and the new calculated area should be less than 1%::::Разница между новой официальной площади участка и новой вычисленной площади не должны быть более 1%.::::..القرث بين مساحة القطعة الجديدو الرسمي والمساحة الجديدة المحتسبة يجب ان لا يتجاوز 1%::::La différence entre la superficie officielle de la nouvelle parcelle et la superficie calculée doit être inférieur à 1%.', '#{id}(cadastre.cadastre_object.id) is requested. 
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('cadastre-object-check-name', 'sql', 'The parcel (cadastre object) should have a valid form of description (appellation)::::Участок (кадастровый объект) должен иметь корректное описание (наименование).::::القطعة او الكائن المكاني يجب ان يكون لها  وصف او تسمية صحيحة::::La parcelle (objet cadastrale) doit avoir une forme de descriptif (appellation) valide.', '#{id}(cadastre.cadastre_object.id) is requested');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('cadastre-redefinition-union-old-new-the-same', 'sql', 'The union of the new polygons must be the same as the union of the old polygons::::Объединение новых полигонов должно быть таким же как объединение старых полигонов.::::الاتحاد من المضلعات الجديدة يجب أن تكون نفس الاتحاد من المضلعات القديمة::::L''union des nouveaux polygones doit être le même que l''union des anciens polygones.', '#{id} is the parameter asked. It is the transaction id.');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('new-cadastre-objects-do-not-overlap', 'sql', 'The new parcel polygons must not overlap::::Новые участки не должны пересекаться.::::مضلعات القطعة الجديدة يجب ان لا تتداخل::::Les polygones des nouvelles parcelles ne doivent pas se superposer.', '#{id}(transaction_id) is requested. Check the union of new co has the same area as the sum of all areas of new co-s, which means the new co-s don''t overlap');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('new-cadastre-objects-present', 'sql', 'New cadastral objects must be defined::::Новые кадастровые объекты должны быть определены.::::يجب تحديد كائن مساحة جديد::::Les nouveaux objets cadastres doivent être définis.', '#{id}(transaction_id) is requested');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('target-parcels-check-isapolygon', 'sql', 'The union of the target parcels must be a polygon::::Объединение целевых участков должно быть полигоном.::::الاتحاد بين القطع يجب ان يكون مضلع::::L''union des parcelles cibles doit être un polygone.', '#{id}(cadastre.cadastre_object.transaction_id) is requested');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('target-parcels-check-nopending', 'sql', 'There should be no pending changes for any of target parcels::::Целевые участки не должны иметь каких-либо незавершенных изменений.::::يجب ان لا يوجد اية تغييرات معلقة على القطعة المستهدفة::::Il ne doit pas y avoir de changement en attente pour aucune des parcelles cibles.', '#{id}(cadastre.cadastre_object.id) is requested');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('target-parcels-present', 'sql', 'Target parcel(s) must be selected::::Целевые участки должны быть выбраны.::::يجب اختيار القطعة المستهدفة::::Le(s) parcelle(s) doivent être sélectionnées.', '#{id}(transaction_id) is requested');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('survey-points-present', 'sql', 'There are at least 3 survey points present::::По крайней мере должны быть определены 3 точки съемки.::::هناك على الاقل ثلاث نقاط مساحة موجودة::::Il doit y avoir au moins 3 points de levé.', '#{id}(transaction_id) is requested. Check there are survey points attached with the cadastre change.
