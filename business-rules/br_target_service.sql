...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('power-of-attorney-owner-check', 'sql', 'Name of person identified in Power of Attorney should match a (one of the) current owner(s)::::Имя лица указанного в доверенности должно совпадать с именем одного из владельцев.::::الاسم المحدد في الوكالة يجب ان يطابق احد اسماء المالكين::::Le nom de la personne identifiée dans la procuration doit correspondre à un des propriétaires actuels.', '#{id}(application.service.id)');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('service-has-person-verification', 'sql', 'Within the application for the service a personal identification verification should be attached.::::В заявлении должен присутствовать документ удостоверяющий личность.::::ضمن الطلب ولاجل الخدمة يجب  ارفاق وثيقة تعريف بالشخصية::::Pour la demande d''un service, la vérification de l''identification personnelle doit être attachée.', '#{id}(application.service.id) is requested');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('document-supporting-rrr-is-current', 'sql', 'Documents supporting rights (or restrictions) must have current status::::Документы использующиеся для регистрации прав или ограничений должны иметь текущий (активный) статус.::::الوثائق التي تدعم الحقوق او القيود يجب ان يكون لها الحالة الحالية::::Les documents justificatifs de droits (ou restrictions) doivent avoir le statut courant.', '#{id}(application.service.id)');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('power-of-attorney-service-has-document', 'sql', 'Service ''req_type'' must have must have one associated Power of Attorney document::::Услуга ''req_type''  должна иметь одну доверенность.::::خدمة ''req_type'' يجب ان ترتبط بوكالة::::Le service ''req_type'' doit avoir un document de procuration associé.', '#{id}(application.service.id)');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('required-sources-are-present', 'sql', 'All documents required for the service ''req_type'' are present.::::Должны присутствовать все документы необходимые для услуги ''req_type'' .::::جميع الوثائق المطلوبة لخدمة  ''req_type''  موجودة::::Tous les documents requis pour le service ''req_type'' sont présents.', 'Checks that all required documents for any of the services in an application are recorded. Null value is returned if there are no required documents');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('service-on-complete-without-transaction', 'sql', 'Service ''req_type'' must have been started and some changes made in the system::::Услуга ''req_type''  должна быть запущена и сделаны какие-либо изменения.::::يجب ان تكون  اخدمة ''req_type'' قد بدأت وهناك تغييرات غلى النظام::::Le service ''req_type'' doit avoir commencé et des changements faits dans le système.', '');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('service-check-no-previous-digital-title-service', 'sql', 'For the Convert Title service there must be no existing digital title record (including the recording of a primary (ownership) right) for the identified title::::Для услуги конвертации недвижимости не должно быть существующих записей этой недвижим.::::في خدمة تحويل الى سند  ملكية رقمي يجب ان لا يوجد  سجل رقمي للملكية بما فيه  تسجيل الحق الاساسي::::Pour le service de conversion de titre, il ne doit pas y avoir de titre numérique existant déjà enregistré (incluant l''enregistrement d''un droit principal (propriété) pour le titre identifié.', '#{id}(application.service.id) is requested where service is for newDigitalTitle or newDigitalProperty');
