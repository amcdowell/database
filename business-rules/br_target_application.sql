...
VALUES ('application-br6-check-new-title-service-is-needed', 'sql', 'An application can be associated with a property which should have a digital title record.::::Non esiste un formato digitale per questa proprieta. Aggiungere un Nuovo Titolo Digitale alla vostra pratica::::Заявление должно содержать недвижимость которая имеет электронную запись в базе.', 'Rule checks to see if there is a ba_unit record for the property identified for the application at lodgement');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('applicant-name-to-owner-name-check', 'sql', 'The applicants name should be the same as (one of) the current owner(s)::::Il nome del richiedente differisce da quello dei proprietari registrati::::Имя заявителя должно быть таким же как у одного из текущих владельцев.', '#{id}(application.application.id) is requested');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('application-on-approve-check-public-display', 'sql', 'The publication period must be completed.::::Il periodo di pubblica notifica deve essere completato::::Период публикации должен быть завершен.', 'Checks the completion of the public display period for all instances of systematic registration service related to the application');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('app-check-title-ref', 'sql', 'Invalid identifier for title::::ITALIANO::::Неверный идентификатор права собственности', '');
...
INSERT INTO system.br_validation(br_id, target_code, target_application_moment, severity_code, order_of_execution) 
VALUES ('app-check-title-ref', 'application', 'validate', 'medium', 750);

----------------------------------------------------------------------------------------------------

INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('application-br1-check-required-sources-are-present', 'sql', 'All documents required for the services in this application are present.::::Sono presenti tutti i documenti richiesti per il servizio::::Все документы, необходимые для указанных услуг должны присутствовать в заявлении.', 'Checks that all required documents for any of the services in an application are recorded. Null value is returned if there are no required documents');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('application-br8-check-has-services', 'sql', 'An application must have at least one service::::La Pratica ha almeno un documento allegato::::Заявление должно иметь хотя бы одну услугу.', 'Checks that an application has at least one service. When this rule fails you should add a service to the application or cancel the application.');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('app-title-has-primary-right', 'sql', 'A single primary right (such as ownership) must be identified whenever a new title record is created::::Il titolo originario del nuovo titolo deve avere un diritto primario::::Единственное право собственности должно быть зарегистрировано для нового объекта недвижимости.', '#{id}(application.application.id) is requested');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('application-verifies-identification', 'sql', 'Personal identification verification should be attached to the application.::::Non esistono dettagli identificativi registrati per la pratica::::Персональный идентификационный документ должен быть прикреплен к заявлению.', '#{id}(application.application.id) is requested');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('app-allowable-primary-right-for-new-title', 'sql', 'An allowable primary right (ownership, apartment, State ownership, lease) must be identified for a new title::::Un diritto primario disponibile (proprieta, appartamento o proprieta statale) deve essere identificato per un nuovo titolo::::Одно из допустимых прав собственности такое как - право собственности, право собственности на квартиру, государственное право собственности, аренда, должно быть зарегистрировано для нового объекта недвижимости.', '#{id}(application.application.id) is requested');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('application-on-approve-check-services-status', 'sql', 'All services in the application must have the status ''cancelled'' or ''completed''.::::Tutti i servizi devono avere lo stato di cancellato o completato::::Все услуги в заявлении должны иметь статус ''Отменен'' или ''Завершен''.', 'Checks the service.status_code for all instances of service related to the application');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('application-approve-cancel-old-titles', 'sql', 'An application including a new freehold service must also terminate the parent title(s) with a cancel title service.::::Identificati titoli esistenti. Prego terminare i titoli esistenti usando il servizio di Cancellazione Titolo::::An application including a new freehold service must also terminate the parent title(s) with a cancel title service.', '');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('application-on-approve-check-systematic-reg-no-objection', 'sql', 'There must be no objection against the systematic registration::::Non ci devono essere obiezioni attive::::Не должно быть каких-либо обжалований по системной регистрации.', 'Checks the absence of objections for systematic registration service related to the application');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('newtitle-br24-check-rrr-accounted', 'sql', 'All rights and restrictions from existing title(s) must be accounted for in the new titles created in this application.::::non tutti i diritti e le restrizioni sono stati trasferiti al nuovo titolo::::Все права и ограничения существующих объектов недвижимости должны быть учтены в новых объектах недвижимости, создаваемых в этом заявлении.', '#{id}(application_id) is requested');
...
INSERT INTO system.br_validation(br_id, target_code, target_application_moment, severity_code, order_of_execution) 
VALUES ('newtitle-br24-check-rrr-accounted', 'application', 'validate', 'critical', 160);

----------------------------------------------------------------------------------------------------

INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('application-for-new-title-has-cancel-property-service', 'sql', 'When a new title is created there must be a cancel title service in the application for the parent title.::::Non esiste nella pratica un servizio di cancellazione titolo. Aggiungere questo servizio alla pratica::::Когда создается новый объект недвижимости, должна присутствовать услуга ликвидации недвижимости в заявлении родительской недвижимости.', '');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('application-cancel-property-service-before-new-title', 'sql', 'New Freehold title service must come before Cancel Title service in the application.::::Il servizio di cancellazione titolo deve venire prima di quello di creazione nuovo titolo. Cambiare ordine servizi nella pratica::::Услуга нового права собственности (свободное) должна быть перед услугой отмены права собственности.
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('app-current-caveat-and-no-remove-or-vary', 'sql', 'The identified property has a current or pending caveat registered on the title. The application must include a cancel or waiver/vary caveat service for registration to proceed.::::Per questo titolo siste un diritto di prelazione pendente o corrente e la pratica non include un servizio di cancellazione o di variazione prelazione ::::Выбранная недвижимость имеет арест. Заявление должно включать услугу снятия ареста для того чтобы продолжить регистрацию.', '#{id}(application.application.id) is requested. It checks if there is a caveat (pending or current) registered
