...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('applicant-name-to-owner-name-check', 'sql', 'The applicants name should be the same as (one of) the current owner(s)::::Имя заявителя должно быть таким же как у одного из текущих владельцев.::::مقدم الطلب يجب ان يكون واحد من المالكين الحاليين::::Le nom d''au moins un des propriétaires actuels doivent apparaître parmis les demandeurs.', '#{id}(application.application.id) is requested');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('application-br6-check-new-title-service-is-needed', 'sql', 'An application can be associated with a property which should have a digital title record.::::Заявление должно содержать недвижимость которая имеет электронную запись в базе.::::الطلب يجب ارتباطه بملكية لها سند ملكية الكتروني::::Une demande peut être associée à une propriété qui doit avoir une trace de titre numérique.', 'Rule checks to see if there is a ba_unit record for the property identified for the application at lodgement');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('application-on-approve-check-systematic-reg-no-objection', 'sql', 'There must be no objection against the systematic registration::::Не должно быть каких-либо обжалований по системной регистрации.::::يجب ان لا يوجد اعتراض على التسجيل المنتظم::::Il ne doit pas y avoir d''objection à l''enregistrement systématique.', 'Checks the absence of objections for systematic registration service related to the application');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('application-br8-check-has-services', 'sql', 'An application must have at least one service::::Заявление должно иметь хотя бы одну услугу.::::الطلب يجب ان يحتوي على خدمة واحدة على الاقل.::::La demande doit contenir au moins un service', 'Checks that an application has at least one service. When this rule fails you should add a service to the application or cancel the application.');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('application-verifies-identification', 'sql', 'Personal identification verification should be attached to the application.::::Персональный идентификационный документ должен быть прикреплен к заявлению.::::يجب ارفاق وثائق التعريف الشخصية مع الطلب::::La vérification de l''identité personnelle doit être attachée à la demande.', '#{id}(application.application.id) is requested');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('application-br1-check-required-sources-are-present', 'sql', 'All documents required for the services in this application are present.::::Все документы, необходимые для указанных услуг должны присутствовать в заявлении.::::جميع الوثائق المطلوبة للخدمة  موجودة::::Tous les documents requis pour les services de cette demande sont présents.', 'Checks that all required documents for any of the services in an application are recorded. Null value is returned if there are no required documents');
