...
VALUES ('application-br8-check-has-services', 'sql', 'An application must have at least one service::::Заявление должно иметь хотя бы одну услугу.::::الطلب يجب ان يحتوي على خدمة واحدة على الاقل.', 'Checks that an application has at least one service. When this rule fails you should add a service to the application or cancel the application.');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('application-br7-check-sources-have-documents', 'sql', 'Documents lodged with an application should have a scanned image file (or other source file) attached::::Документы присутствующие в заявлении должны иметь прикрепленную отсканированную копию.::::الوثائق المرتبطة بالطلب يجب ان يكون لها صور ممسوحة ومرفقة', 'Checks that each document lodged with the application has a scanned image file (or other digital source file) stored in the SOLA database. To remedy the failure of the business rule add the scanned image to the document record through the Document Tab in the Application form or use the Document Toolbar item in the Main form.');
...
system.br.application-approve-cancel-old-titles.feedback Заявление с услугой нового права собственности (свободное) должно также включать услугу ликвидации родительского права собственности.::::خدمة تسجيل ملكية حرة يجب ان تسبق خدمة الغاء ملكية في الطلب', '');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('applicant-name-to-owner-name-check', 'sql', 'The applicants name should be the same as (one of) the current owner(s)::::Имя заявителя должно быть таким же как у одного из текущих владельцев.::::مقدم الطلب يجب ان يكون واحد من المالكين الحاليين', '#{id}(application.application.id) is requested');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('application-on-approve-check-public-display', 'sql', 'The publication period must be completed.::::Период публикации должен быть завершен.::::يجب استكمال فترة النشر', 'Checks the completion of the public display period for all instances of systematic registration service related to the application');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('application-on-approve-check-systematic-reg-no-objection', 'sql', 'There must be no objection against the systematic registration::::Не должно быть каких-либо обжалований по системной регистрации.::::يجب ان لا يوجد اعتراض على التسجيل المنتظم', 'Checks the absence of objections for systematic registration service related to the application');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('application-verifies-identification', 'sql', 'Personal identification verification should be attached to the application.::::Персональный идентификационный документ должен быть прикреплен к заявлению.::::يجب ارفاق وثائق التعريف الشخصية مع الطلب', '#{id}(application.application.id) is requested');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('application-br1-check-required-sources-are-present', 'sql', 'All documents required for the services in this application are present.::::Все документы, необходимые для указанных услуг должны присутствовать в заявлении.::::جميع الوثائق المطلوبة للخدمة  موجودة', 'Checks that all required documents for any of the services in an application are recorded. Null value is returned if there are no required documents');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('application-approve-cancel-old-titles', 'sql', 'An application including a new freehold service must also terminate the parent title(s) with a cancel title service.::::Identificati titoli esistenti. Prego terminare i titoli esistenti usando il servizio di Cancellazione Titolo::::الطلب الذي يحتوي على خدمة  تملك حر يجب ان يحتوي على خدمة اخرى تلغي جميع سندات الملكية السابقة', '');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('app-check-title-ref', 'sql', 'Invalid identifier for title::::Неверный идентификатор права собственности::::معرف السند غير صحيح', '');
