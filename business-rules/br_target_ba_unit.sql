...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('newtitle-br22-check-different-owners', 'sql', 'Owners of new titles should be the same as owners of underlying titles::::Владельцы новых объектов недвижимости должны быть такие же как в родительских объектах.::::المالكون في سند الملكية الجديد يجب ان يكونوا نفس المالكين من السندات السابقة', '#{id}(baunit_id) is requested.
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('ba_unit-has-cadastre-object', 'sql', 'Title must have an associated parcel (or cadastre object)::::Недвижимость должна иметь земельный участок (или кадастровый объект).::::سند الملكية يجب ان يحتوي على قطعة  او كائن مساحة', '#{id}(administrative.ba_unit.id) is requested');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('ba_unit-spatial_unit-area-comparison', 'sql', 'Title area should only differ from parcel area(s) by less than 1%::::Площадь недвижимости не должна отличаться от ее земельного участка более чем на 1%.::::المساحة في سند الملكية لا يجب ان تختلف باكثر من 1% من مساحة القطعة (القطع)', '#{id}(administrative.ba_unit.id) is requested');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('ba_unit-has-a-valid-primary-right', 'sql', 'A title must have a valid primary right::::Недвижимость должна иметь первичное право собственности.::::سند الملكية يجب ان يحتوي على حق اساسي صالح', '#{id}(baunit_id) is requested.');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('ba_unit-has-compatible-cadastre-object', 'sql', 'Title should have compatible parcel (or cadastre object) description (appellation)::::Недвижимость должна иметь совместимый тип земельного участка.::::سند الملكية يجب ان يحتوي على وصف\ اسم   قطعة متوافقة', '#{id}(administrative.ba_unit.id) is requested');
