...
VALUES ('rrr-must-have-parties', 'sql', 'These rights (and restrictions) must have a recorded party (or parties)::::Данные права (или ограничения) должны иметь правообладателей.::::هذه الحقوق او القيود يجب ان يكون عليها اطراف معرفون', '');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('rrr-shares-total-check', 'sql', 'The sum of the shares (in ownership rights) must total to 1::::Сумма долей права собственности должна ровняться 1.::::مجموع الحصص في اصحاب الحقوق يجب ان يساوي واحد صحيح', '#{id}(administrative.rrr.id) is requested');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('rrr-has-pending', 'sql', 'There are no other pending actions on the rights and restrictions being changed or removed on this application::::Нет никаких изменений в правах и ограничениях, сделанных из текущего заявления::::لا يوجد حركات بحاجة لتنفيذ على الحقوق او القيود لهذا الطلب', '#{id}(administrative.rrr.id) is requested. It checks if for the target rrr there is already a pending edit or record.');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('ba_unit-has-caveat', 'sql', 'Caveat should not prevent registration proceeding.::::Арест не должен ограничивать дальнейшую регистрацию.::::القيود لا يجب ان تعطل عملية التسجيل', '#{id}(administrative.rrr.id) is requested.');
...
INSERT INTO system.br(id, technical_type_code, feedback, technical_description) 
VALUES ('ba_unit-has-several-mortgages-with-same-rank', 'sql', 'The rank of a new mortgage must not be the same as an existing mortgage registered on the same title::::Рейтинг новой ипотеки не должен быть таким же как у существующей ипотеки для данной недвижимости.::::رتبة الرهن يجب ان لا تساوي نفس رتبة الرهن  الحالي', '#{id}(administrative.rrr.id) is requested.');
